/*
 * SPDX-FileCopyrightText: NVIDIA CORPORATION & AFFILIATES
 * Copyright (c) 2025 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
 * SPDX-License-Identifier: Apache-2.0
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#ifndef RDK_SERVICES_PROTOCOL_RTP_PARSER_H_
#define RDK_SERVICES_PROTOCOL_RTP_PARSER_H_

/* Platform specific headers and declarations */
#if defined(_WIN32)
#include <ws2tcpip.h>
#pragma comment(lib, "Ws2_32.lib")
#elif defined(__linux__)
#include <arpa/inet.h>
#endif

#include "rdk/services/protocol/protocol_parsers.h"
#include "rdk/services/protocol/rtp.h"
#include "rdk/services/media/media_defs.h"

namespace rivermax
{
namespace dev_kit
{
namespace services
{
/**
 * @brief: RTP protocol parser.
 *
 * Parses RTP headers. Validates RTP version and CSRC limits.
 * @note: Handles basic RTP headers only.
 */
class RTPParser : public TypedProtocolParser<RTPHeader>
{
public:
    size_t get_header_size(const byte_t* data) const override
    {
        if (!data) {
            return RTP_HEADER_SIZE;
        }

        const RTPHeader* rtp_header = get_typed_header(data);
        uint8_t csrc_count = rtp_header->vpxcc & 0x0F;
        size_t header_size = RTP_HEADER_SIZE + (csrc_count * RTP_HEADER_CSRC_GRANULARITY_BYTES);

        bool has_extension = (rtp_header->vpxcc & 0x10) != 0;
        if (has_extension) {
            if (data + header_size + 4 <= data + header_size) {
                return header_size;
            }

            const uint16_t* ext_length_ptr = reinterpret_cast<const uint16_t*>(data + header_size + 2);
            uint16_t ext_length_words = ntohs(*ext_length_ptr);
            header_size += 4 + (ext_length_words * 4);
        }

        return header_size;
    }

    bool validate(const byte_t* data, size_t available_length) const override
    {
        if (available_length < RTP_HEADER_SIZE) {
            return false;
        }

        const RTPHeader* rtp_header = get_typed_header(data);

        uint8_t version = (rtp_header->vpxcc & RTP_VERSION_MASK);
        if (version != RTP_VERSION_2) {
            return false;
        }

        uint8_t csrc_count = rtp_header->vpxcc & 0x0F;
        size_t calculated_size = RTP_HEADER_SIZE + (csrc_count * RTP_HEADER_CSRC_GRANULARITY_BYTES);
        if (available_length < calculated_size) {
            return false;
        }

        bool has_extension = (rtp_header->vpxcc & 0x10) != 0;

        if (has_extension) {
            if (available_length < calculated_size + 4) {
                return false;
            }

            const uint16_t* ext_length_ptr = reinterpret_cast<const uint16_t*>(data + calculated_size + 2);
            uint16_t ext_length_words = ntohs(*ext_length_ptr);
            size_t total_ext_size = 4 + (ext_length_words * 4);
            if (available_length < calculated_size + total_ext_size) {
                return false;
            }
        }
        return true;
    }

    std::string get_protocol_name() const override
    {
        return "RTP";
    }
};

} // namespace services
} // namespace dev_kit
} // namespace rivermax

#endif // RDK_SERVICES_PROTOCOL_RTP_PARSER_H_
