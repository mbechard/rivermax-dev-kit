...
std::pair<bool, uint16_t> parse_fmtp_uint_param(const std::string &token, const std::string &pname, std::string::size_type &pos) {
  std::string param_value{token.substr(pos)};
  try {
    int val = std::stoi(param_value.substr(pname.size()));
    if (val >= 0) {
      return {true, static_cast<uint16_t>(val)};
    }
    std::cerr << "Attribute a=fmtp has unexpected negative paramater value " << token.substr(0, token.size()-1) << "." << std::endl;
  }
  catch (const std::invalid_argument &) {
    std::cerr << "Attribute a=fmtp has invalid parameter value " << token.substr(0, token.size()-1) << "." << std::endl;
  }
