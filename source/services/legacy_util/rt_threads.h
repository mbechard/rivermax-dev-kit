...
std::pair<bool, uint16_t> parse_fmtp_uint_param(const std::string &, const std::string &, std::string::size_type &);
    ...
    try {
        if (is_2022_6) {
            ...
            if (frame_rate_string.find("/") != std::string::npos) {
                ...
                frame_rate = std::stoi(nume_deno[0]);
                int denominator = std::stoi(nume_deno[1]);
                if (denominator == 0) {
                    std::cerr << "invalid framerate denominator (division by zero): " << frame_rate_string << std::endl;
                    return false;
                }
                frame_rate /= denominator;
            } else {
                frame_rate = std::stoi(frame_rate_string);
            }
        }
        return true;
    } catch (const std::exception& e) {
        std::cerr << "error parsing framerate: " << frame_rate_string << " (" << e.what() << ")" << std::endl;
        return false;
    }
}
...
bool parse_video_fmtp_param(const std::string &sdp, T &stream_data, size_t &pos_start, size_t &pos_end)
{
    ...
    for (const auto& token : line_vec) {
        if ((pos = token.find("depth=")) != std::string::npos) {
            auto p = parse_fmtp_uint_param(token, "depth=", pos);
            if (p.first) {
                stream_data.depth = p.second;
                continue;
            }
            return false;
        }
        ...
        if ((pos = token.find("width=")) != std::string::npos) {
            auto p = parse_fmtp_uint_param(token, "width=", pos);
            if (p.first) {
                stream_data.width = p.second;
                continue;
            }
            return false;
        }
        if ((pos = token.find("height=")) != std::string::npos) {
            auto p = parse_fmtp_uint_param(token, "height=", pos);
            if (p.first) {
                stream_data.height = p.second;
                continue;
            }
            return false;
        }
        ...
        if ((pos = token.find("CMAX=")) != std::string::npos) {
            auto p = parse_fmtp_uint_param(token, "CMAX=", pos);
            if (p.first) {
                stream_data.cmax = p.second;
                continue;
            }
            return false;
        }
